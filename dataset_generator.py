...
def generate_synthetic_data(n_students=200, n_weeks=20):
    print(f"Generating data for {n_students} students over {n_weeks} weeks...")
    ...
    for i in range(n_students):
        student_id = f's_{1000 + i}'
        persona_name = np.random.choice(list(personas.keys()), p=[0.25, 0.3, 0.2, 0.25])
        ...
        for week in range(1, n_weeks + 1):
            ...
            gpa_base_score = (base_prev_gpa * 0.4) + (persona['prev_gpa_mean'] * 0.6)
            study_effect = (self_study_hours + library_days * 2 - persona['study_hours']) * 0.02
            sleep_effect = (sleep_hours - persona['sleep_hours']) * 0.05
            stress_effect = (persona['stress'] - stress) * 0.1
            
            semester_gpa = gpa_base_score + study_effect + sleep_effect + stress_effect + \
                           np.random.normal(0, persona['gpa_noise'])
            semester_gpa = np.clip(semester_gpa, 0, 4.0)

            at_risk_prob = 0.1 + (stress / 5) * 0.5 - (semester_gpa / 4) * 0.4
            at_risk_next_month = 1 if (np.random.rand() < at_risk_prob and (stress > 4 or semester_gpa < 2.5)) else 0
            ...
            student_data.append({
                ...
                'at_risk_next_month': at_risk_next_month
            })
